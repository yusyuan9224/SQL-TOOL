...
class MSSQLScreen:
    def __init__(self, root, login_app):
        ...
        self.sql_login = login_app.sql_login  # 確保 sql_login 可訪問
        self.frame = tk.Frame(self.root)
        self.frame.pack(fill=tk.BOTH, expand=True)

        self.label = tk.Label(self.frame, text="Welcome to MS SQL Server Database", font=("Helvetica", 16))
    ...
    def on_closing(self):
        # Close the SQL connection if window is closed
        if self.sql_login.connection is not None:
            self.sql_login.connection.close()
            self.sql_login.connection = None
        self.root.destroy()
